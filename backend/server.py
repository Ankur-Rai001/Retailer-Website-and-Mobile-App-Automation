...
@api_router.get("/ondc/kyc-status")
...
@api_router.post("/ondc/sync-catalog")
async def sync_catalog_to_ondc(user: User = Depends(get_current_user)):
    try:
        ...
        # Initialize ONDC integration
        import sys
        sys.path.append(str(Path(__file__).parent / "utils"))
        from ondc_integration import ONDCIntegration
        
        # Create ONDC instance (in production, these would be from env/config)
...
@api_router.get("/ondc/sync-status")
...
# ONDC Beckn Protocol Webhooks (for receiving orders from buyer apps)
@api_router.post("/ondc/webhooks/search")
async def ondc_search_webhook(request: Request):
    """Handle ONDC search requests"""
    try:
        ...
        # Get matching products from all stores with ONDC enabled
        stores = await db.stores.find({"ondc_enabled": True}, {"_id": 0}).to_list(100)
        
        all_providers = []
        for store in stores:
            products = await db.products.find(
                {"store_id": store["store_id"], "is_active": True, **search_filter},
                {"_id": 0}
            ).to_list(100)
...
async def ondc_select_webhook(request: Request):
    ...
    try:
        ...
        response = ondc.create_select_response(order.get("items", []), store)
        return response
...
async def ondc_init_webhook(request: Request):
    ...
    try:
        ...
        response = ondc.create_init_response(order, billing_info)
...
async def ondc_confirm_webhook(request: Request):
    ...
    try:
        ...
        response = ondc.create_confirm_response(order_id, order)
